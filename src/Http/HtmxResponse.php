...
use HTMx\Services\Element2String;
...
class HtmxResponse extends Response
{
    ...
    public function renderFragment(string $element, array $args = [], ?string $pkgHandle = null): static
    {
        $this->fragments = [(string) new Element2String($element, $pkgHandle, $args)];

        return $this;
    }

    public function addFragment(string $element, array $args = [], ?string $pkgHandle = null): static
    {
        $this->fragments[] = (string) new Element2String($element, $pkgHandle, $args);
    ...
    {
        if (count($triggers) !== count(array_filter($triggers, 'is_null'))) {
